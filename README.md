...
```bash
cmake --build out/linux-clang --target web_transport_test_server
...
服务端提供两个 WebTransport endpoint：

- `https://localhost:6121/webtransport/echo`
- `https://localhost:6121/webtransport/devious-baton`
- baton endpoint 还接受 `count` 和 `baton` 查询参数，例如
  `?count=3&baton=42`

上游没有提供原生 WebTransport demo client。可以在信任测试证书的浏览器中
通过 JavaScript WebTransport API 连接 echo endpoint：

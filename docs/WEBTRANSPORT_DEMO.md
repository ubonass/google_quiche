...
### 2.8 1-RTT Null Crypter 抓包模式

Demo 提供仅用于本地抓包分析的 `--null_one_rtt_crypter` 参数。client 和 server
必须同时设置为 `true`。该模式保持标准 Initial 和 TLS Handshake 加密，禁用 0-RTT，
并使用 QUICHE `NullEncrypter`/`NullDecrypter` 处理 1-RTT packet，因此 HTTP/3 和
WebTransport应用数据不具备机密性。

该模式不是标准 QUIC，不支持与其他 QUIC/WebTransport 实现互操作，也不能用于
生产环境、互联网或敏感数据。标准 Wireshark QUIC dissector 不能自动展开 Null
Crypter 的 1-RTT payload，但可以在 UDP payload 中搜索已知 Echo 内容。

Echo 两端会输出 `echo_send`、`echo_receive` 和 `echo_verify` 日志。payload 中的
不可打印字符会被转义，超过显示上限时标记 `truncated=true`。

对于过短的 1-RTT packet，Demo 会在 Null ciphertext 前增加仅用于满足 Header
Protection 采样长度的8字节前缀；接收端在 Null hash校验前剥离该前缀，不会向
QUIC plaintext、Echo或其他 WebTransport应用 payload 增加任何字节。
...
两端参数必须一致。只在一端开启时，第一个 1-RTT packet 无法被对端正确处理，连接
通常会在 HTTP/3 SETTINGS 或 WebTransport session ready 阶段超时。

### 3.8 使用 Devious Baton endpoint
